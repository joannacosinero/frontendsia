...
<template>
    <div>
        ...
        <b-modal id="editRecord" title="Record Entry" ok-title="Save Record" @ok="onSubmit">
           <form action="#" >
                <b-form-group 
                label="Name"
                label-for="name"
...
export default {
  ...
  methods:{
        async onSubmit() {
            this.$axios.put('https://lentrix.tk/cosinero/api/records/' + this.record.id, this.record)
            .then((res)=>{
                if(res.status==202) {
